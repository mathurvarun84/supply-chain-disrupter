...
PROMPT_LOG = [
    {"ts": "14:32:14", "agent": "L7", "model": "gpt-4o",
     ...
     "full_prompt": "SYSTEM:\nYou are a supply chain mitigation specialist.\n\nUSER:\nGenerate mitigation plan for CRITICAL disruption: Taiwan earthquake M7.2 affecting Hsinchu fab cluster. Active record: order_id=4821, port=Taiwan, sku=GPU-A100, delivery_status=Shipping delayed. Risk score: 0.94 CRITICAL. Simulation shows 78% stockout probability within 14 days.",
     "resp": '{"urgency":"IMMEDIATE","actions":[{"rank":1,"text":"Reroute Cape of Good Hope via Singapore hub"},{"rank":2,"text":"Activate alternate supplier TSMC Arizona fab allocation"}],"cost_estimate":"$2.4M incremental freight","rag_citations":["Taiwan fab disruption playbook 2024"]}',
    ...
    {"ts": "14:32:07", "agent": "L4", "model": "gpt-4o",
     ...
     "full_prompt": "SYSTEM:\nYou are the LLM-as-Judge in a three-signal supply-chain risk ensemble.\n\nUSER:\nClassify supply chain risk given signals:\n  Rule-based: HIGH (geo=0.71, supply=0.89, freight=0.54)\n  DistilBERT: CRITICAL (confidence=0.91)\n  LLM Signal: CRITICAL (driver=seismic_event, confidence=high)\n  Record: order_id=4821, delivery_status=Shipping delayed, export_control_level=4.2",
     "resp": '{"final_label":"CRITICAL","verdict_type":"consensus","reasoning":"All three signals agree on CRITICAL. Seismic event in Hsinchu cluster with shipping delays confirms escalation.","signals_agreed":true,"final_critical_flag":true}',
    ...
    {"ts": "14:31:58", "agent": "L2", "model": "gpt-4o-mini",
     ...
     "full_prompt": "SYSTEM:\nYou are a supply chain news analyst for semiconductor procurement.\n\nUSER:\nAnalyze 9 news items for supply chain disruption signals.\nDisruption type: EARTHQUAKE. Affected port: Taiwan. Severity hint: CRITICAL.\nNews headlines: [1] M7.2 earthquake strikes Hsinchu County... [2] TSMC fab operations suspended pending inspection...",
     "resp": '{"category":"NATURAL_DISASTER","severity":"CRITICAL","affected_regions":["Taiwan","Hsinchu"],"news_severity_component":0.71,"expected_duration_days":21,"summary":"Major seismic event disrupting Hsinchu semiconductor cluster."}',
    ...
    {"ts": "14:31:52", "agent": "L3", "model": "gpt-4o-mini",
     ...
     "full_prompt": "SYSTEM:\nYou are a weather risk analyst for semiconductor supply chains.\n\nUSER:\nEvaluate weather impact on 6 fab hub cities.\nHsinchu: wind=62km/h, precip=18.4mm, WMO code=95 (Thunderstorm)\nNumeric severity baseline: 8.7/10. Data source: Open-Meteo live API.",
     "resp": '{"event_classification":"SEVERE_STORM","geo_risk_component":0.52,"affected_semiconductor_hubs":["Hsinchu","Tainan"],"supply_chain_narrative":"Post-earthquake aftershock weather compounding fab access disruption.","rag_escalation_warranted":true}',
